...
ifnull(DMPatientNeedingFollowUP.CountOfPat,0) as 'Number needing follow up by MMU based on MMU physician evaluation',
ifnull(DMNumberSeenByMMUInThisVisit.TotalFollowUpCount,0) as 'DM Patient seen by MMU in this visit',
ifnull(DMTotalPatientIdentifiedInVillage.CountOfPat,0) as 'Total DM patients identified in the village who are eligible for treatment under MMU',
/*DMPatientSeenInThisMonth.CountOfPat as 'DM patients seen this month',*/
ifnull(DMPercentSeenThisMonth.PercentDMPatSeen,0) as '% of follow up DM patients seen this month',
ifnull(NewDMConfirmedPatient.CountOfPat,0) as 'New DM patients seen in this visit',
ifnull(DMNumberOfPatientGivenMedByCHW.Count_Patient_Med_CHW,0) as 'Number of patients whose medicines given to CHW as the pt did not/could not attend the clinic',
ifnull(DMPercentMedsGivenToCHW.PercentDMCHW,0) as '% number of patients whose medicines given to CHW',
ifnull(DMPatientAbsentInLastThreeVisits.CountOfAbsentPat,0) as 'Number of patients who discontinued treatment from MMU',
ifnull(DMPercentPatientsDiscontinuedTreatment.PercentDMDiscontinued,0) as '% number of patients who discontinued treatment from MMU',
ifnull(DMPatientWithSideEffects.Count_Patient_ADR,0) as 'DM patients reporting side effects due to medicines',
ifnull(DMPersentPatientWithSideEffects.PercentDMPatientWithSideEffects,0) as '% DM patients reporting side effects due to medicines',
ifnull(DMPatientWithSAE.Count_Patient_SAE,0) as 'Serious adverse event due to DM medicines',
ifnull(DMPersentPatientWithSAE.PercentDMPatientWithSideEffects,0) as '% DM patients reporting SAE',
ifnull(DMPatientDeath.CountOfDeadPat,0) as 'Number of DM patients whose deaths were reported this month',
ifnull(DMPersentPatientDeath.PercentDMDead,0) '% Number of DM patients deaths'
from 
(
/*Village*/
